	...
	for($j=0;$j<7;$j++){
		for($k=0;$k<=30;$k++){
			...
			foreach($xml as $games){
				...
				$content = "";
				$fp = fopen("http://gd2.mlb.com/components/game/mlb/year_".$year[$i]."/month_".$month[$j]."/day_".$day[$k]."/".$gstring2."/inning/inning_all.xml","rb");
				if (!$fp)
					break;
				while (!feof($fp))
				...
				fclose($fp);
				//寫入local computer
				$fp=fopen("D:/xampp/htdocs/www/mlb_database/inning_all/".$year[$i]."/".$gstring2."_inning_all.xml", "w+");
				fwrite($fp, $content);
				fclose($fp);
			}
		}
