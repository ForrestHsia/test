	...
	for($j=0;$j<9;$j++){
		for($k=0;$k<=30;$k++){
			...
			foreach($xml as $games){
				...
				$content = "";
				$fp = fopen("http://gd2.mlb.com/components/game/mlb/year_".$year[$i]."/month_".$month[$j]."/day_".$day[$k]."/".$gstring2."/players.xml","rb");
				if (!$fp)
					break;
				while (!feof($fp))
				...
				fclose($fp);
				//寫入local computer
				$fp=fopen("D:/xampp/htdocs/www/mlb_database/players/".$year[$i]."/".$gstring2."_players.xml", "w+");
				fwrite($fp, $content);
				fclose($fp);
			}
		}
